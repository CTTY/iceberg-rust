...
use super::{FileMetadata, FileRead, FileWrite, InputFile, OutputFile, Storage, StorageConfig};
...
/// storage.write("test.txt", Bytes::from("hello")).await?;
/// let data = storage.read("test.txt").await?;
...
// Custom serialization: serialize to empty object
impl Serialize for MemoryStorage {
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where S: Serializer {
        // Serialize as an empty struct - data is not persisted
        use serde::ser::SerializeStruct;
        let s = serializer.serialize_struct("MemoryStorage", 0)?;
        s.end()
    }
}

// Custom deserialization: deserialize to new empty instance
impl<'de> Deserialize<'de> for MemoryStorage {
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where D: Deserializer<'de> {
        // Ignore any serialized data and create a new empty instance
        ...
        impl<'de> serde::de::Visitor<'de> for MemoryStorageVisitor {
            ...
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("a MemoryStorage struct")
            }
            ...
            where A: serde::de::SeqAccess<'de> {
                // Consume any elements but ignore them
        ...
        deserializer.deserialize_any(MemoryStorageVisitor)
    }
}
...
///
/// This struct implements `FileRead` for reading from in-memory data
